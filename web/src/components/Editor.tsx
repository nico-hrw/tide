                ...
                return {
                    ...
                    isGhost: {
                        ...
                        renderHTML: attributes => {
                            if (!attributes.isGhost) return {};
                            return { 'data-is-ghost': 'true' };
                        }
                    },
                };
            },
            renderHTML({ node, HTMLAttributes }) {
                const isGhost = node.attrs.isGhost === true || node.attrs.isGhost === 'true' || String(node.attrs.id).startsWith('ghost-');
                ...
                if (mentionNode) {
                    ...
                    if (targetId) {
                        ...
                        if (onLinkClickRef.current) {
                            const label = mentionNode.textContent?.replace(/^@/, '') || '';
                            onLinkClickRef.current({ id: targetId, type: 'file', title: label });
                        } else {
                            // Fallback to store only if no callback provided
    ...
    useEffect(() => {
        ...
        if (stored) {
            ...
            try {
                ...
                setTimeout(() => {
                    // Always insert a mention (merged from magicLink)
                    ...
                    // Cleanup the original '@' or whatever trigger was used (mention extension handles this mostly, but if we are here we might need manual cleanup of the pending state)
