...
| MobileLayout | `web/src/components/Layout/MobileLayout.tsx` | Mobile shell V3: calendar-primary, left sidebar (folder creation, Prüfungen/Chat/Finance tabs), SmartSearch glass panel (keyboard-aware, event detector via parseGermanDate, **full-text content search** via `window.__mobileNoteCache` loaded from `localStorage` `tide_search_*` keys on each search open — index written by page.tsx after each note decryption), note editor with editable title + SmartSearch bar, liquid-glass bottom smart bar; **allday events strip** below week-header row (GCal-style, row-assigned chips spanning day columns, click → detail sheet); context-menu bottom sheet (long-press note/folder → Öffnen/Umbenennen/Teilen/Löschen); week↔day view toggle; day/agenda view with infinite scroll; note-to-folder drag; `eventDragActive` disables swipe transition during drag; all headers use `env(safe-area-inset-*)`; **note linking removed from SmartSearch**. **Week swipe hold**: horizontal swipe to navigate weeks requires 500 ms hold before activating — quick swipes and vertical scrolls pass through normally. **SmartSearch**: panel uses `maxHeight: 78vh` (auto-sizes to content, scrolls when results overflow); results list has `overscrollBehavior: contain` to prevent sheet dismissal on scroll; events sorted by proximity to current date (closest first); up to 6 event results shown. |
